...
WORLD_NAME="drone_competition_v1" # <worlname="drone_competition_v1">
SIMULATION_GAZEBO_DIR="$HOME/.simulation-gazebo/worlds"
PX4_SIMULATION_GAZEBO_DIR="$HOME/PX4-Autopilot/Tools/simulation/gz"
...
# === 2️⃣ 启动 Gazebo 仿真 ===
if [ ! -f "$SIMULATION_GAZEBO_DIR/simulation-gazebo" ]; then
    echo "⚠️ 未找到 $SIMULATION_GAZEBO_DIR/simulation-gazebo，正在从 PX4-Autopilot 拷贝..."
    # 拷贝 PX4-Autopilot 中的 simulation-gazebo 文件到 ~/.simulation-gazebo/worlds
    cp "$PX4_SIMULATION_GAZEBO_DIR/simulation-gazebo" "$SIMULATION_GAZEBO_DIR/"
    # 确保文件可执行
    chmod +x "$SIMULATION_GAZEBO_DIR/simulation-gazebo"
    echo "✅ 已成功拷贝并设置权限文件到 $SIMULATION_GAZEBO_DIR"
else
    echo "✅ 已找到 $SIMULATION_GAZEBO_DIR/simulation-gazebo，无需拷贝"
fi
...
PX4_GZ_MODEL_POSE='0.5, 0.5' PX4_SIM_MODEL=x290_mono_cam_down_lidar_3d_depth \
